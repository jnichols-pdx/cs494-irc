...
//                    IrcKind::IRC_KIND_HEARTBEAT => {
                    ...
                    let mut rlp = RoomListingPacket::new()?;
                        ...
                        rlp.push(&"Beta".into())?;
                        rlp.push(&"Gamma".into())?;
                        rlp.push(&"bovine_history".into())?;
                        rlp.push(&"r/cats".into())?;
                        rlp.push(&"r/cats".into())?;
                        rlp.push(&"r/cats".into())?;
                        rlp.push(&"r/cats".into())?;
                        rlp.push(&"r/cats".into())?;
                        ...
                        rlp.push(&"r/cats".into())?;
                        rlp.push(&"r/cats".into())?;
                        rlp.push(&"r/Politics".into())?;
                    let mut ulp = UserListingPacket::new()?;
                        ulp.set_room(&"bovine_history".into())?;
                        ulp.push(&"Ada".into())?;
                        ulp.push(&"Ben".into())?;
                        ulp.push(&"Charlie".into())?;
                    let mut ulp2 = UserListingPacket::new()?;
                        ulp2.set_room(&"r/cats".into())?;
                        ulp2.push(&"Maru".into())?;
                        ulp2.push(&"Maru".into())?;
                        ulp2.push(&"Maru".into())?;
                        ulp2.push(&"Maru".into())?;
                        ...
                        ulp2.push(&"Maru".into())?;
                        ulp2.push(&"Maru".into())?;
                        ulp2.push(&"Shadow".into())?;
                        ulp2.push(&"GrumpyCat".into())?;
                    let mut ulp3 = UserListingPacket::new()?;
                        ulp3.set_room(&"r/Politics".into())?;
                    let mut ulp4 = UserListingPacket::new()?;
                        ulp4.set_room(&"r/Politics".into())?;
                    ...
                    let mut pmp2= PostMessagePacket::new(&"r/Politics".into(), &"Ben".into(), &"RA RA go team (blue|red)!!!!".into())?;
                   ...
                   // IrcKind::IRC_KIND_POST_MESSAGE => {
                    ...
                    dude.connection.write(&ulp4.as_bytes()).await?;
                    dude.connection.write(&pmp1.as_bytes()).await?;
                    dude.connection.write(&pmp1.as_bytes()).await?;
                    dude.connection.write(&pmp1.as_bytes()).await?;
                    dude.connection.write(&pmp1.as_bytes()).await?;
